...
const standardizePhoneNumbers = (arr) => {
  const regex = /[0-9]/;
  return arr.map(string => string.split('').filter(nums => regex.test(nums)).join(''));
};
...
const onlyOddChars = (str) => {
  return str.split('').filter( (character, index) => index % 2 !== 0).join('');
};
...
const allHappy = (arr) => {
  return !arr.map(item => item.includes(':)') ? true : false).includes(false);
};
