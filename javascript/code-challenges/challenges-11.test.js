...
function transformToLis(obj){
  return Object.entries(obj).map( pairing => `<li>${pairing[0]}: ${pairing[1]}</li>` );
};
...
const count = (target, input) => {
  return input.reduce((a, b) => a.concat(b), []).filter(el => el === target).length;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  return input.map(array => array.filter(el => (el % 5 === 0 && typeof el === 'number')).map(num => num ? Math.pow(2, num) : []));
};
...
let findShortest = (data) => {
  return data.reduce((previous, current) => Number(current.height) < Number(previous.height) ? current : previous).name;
};
