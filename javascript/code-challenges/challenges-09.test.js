...
const maxInArray = (arr) => {
  return arr.reduce((previous, current) => current > previous ? current : previous);
};
...
const getHouses = (arr) => {
  let houses = [];
  arr.forEach(obj => {houses.push(Object.values(obj.house).reduce((prv, curr) => {return prv + curr}))})
  return houses;
};
...
const hasChildrenValues = (arr, character) => {
  return arr.reduce((last, current) => last || current.name === character && Object.keys(current).includes('children'), false);
};
