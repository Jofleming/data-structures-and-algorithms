...
const screenForNames = (arr) => {
  return arr.filter(element => element.match(/^[MD][rs]s?\.\s[A-Za-z]+/));
};
...
let biggerThanLuke = (arr) => {
  ...
  return arr.reduce( (prev, character) => Number(character.mass) > Number(luke.mass) ? [...prev, character.name] : prev, [] ).join(' - ');
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < winCombo.length; i++) {
    let posOne = winCombo[i][0];
    let posTwo = winCombo[i][1];
    let posThree = winCombo[i][2];
    if (fullBoard[posOne] !== ''
